(ns mambu.main
  "Kotodama WASM entrypoint for the Mambu clean-room actor (L4) — Clojure port.
  ...
  facts (`mambu.<Entity>/<field>`); `*store*` is the in-memory materialization
...
(def entity-specs
  [{:entity "LoanAccount"   :plural "loanaccounts"   :id-prefix "mambu_loa"
    :fields [:encodedKey :accountState :accountSubState :accountHolderType :accountHolderKey :loanAmount :loanName :daysInArrears :creationDate :approvedDate :closedDate]              :required [:encodedKey]
    :coerce {:loanAmount :float :daysInArrears :int}                       :refs {}}
   {:entity "DepositAccount"      :plural "depositaccounts"      :id-prefix "mambu_dep"
    :fields [:encodedKey :accountState :accountHolderType :accountHolderKey :creationDate :approvedDate] :required [:encodedKey]
    :coerce {}          :refs {}}
   {:entity "Client"     :plural "clients"     :id-prefix "mambu_cli"
    :fields [:encodedKey :state :firstName :lastName :emailAddress :mobilePhone :creationDate]        :required [:encodedKey]
    :coerce {} :refs {}}
   {:entity "Group" :plural "groups" :id-prefix "mambu_gro"
    :fields [:encodedKey :groupName :assignedBranchKey :creationDate]          :required [:encodedKey]
    :coerce {}                       :refs {}}
   {:entity "LoanTransaction"    :plural "loantransactions"    :id-prefix "mambu_loa"
    :fields [:encodedKey :type :amount :notes :creationDate :bookingDate :valueDate]  :required [:encodedKey]
...
(defn emit-facts
  "EAVT facts for one record: {\"mambu.<Entity>/<field>\" v ...}. The datomic
...
(defn healthz [] [{:status "ok" :actor "mambu-compat" :tier tier :entities entities} 200])
